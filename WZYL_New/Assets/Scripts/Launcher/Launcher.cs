...
public class Launcher
{
    ...
    public void Init()
    {
        ...
        m_ObjectHandle = GameObject.Instantiate(SceneRuntime.PlayerMgr.LauncherObject(indx)) as GameObject;//克隆发炮平台信息
    ...
    public void UpdateUserGold(byte clientSeat)
    {
        ...
        m_LabelScore.text = string.Format("{0:N3}",((double)(PlayerRole.Instance.GetPlayerGlobelBySeat(clientSeat))/1000f)).ToString().Replace(",", "");//捕鱼游戏场景中的金币位置
    }
    ...
    void OnSubtractButtonMessage(GameObject button)
    {
        ...
        SceneRuntime.SceneLogic.ChangeRate(true);//false
    ...
    void OnAddButtonMessage(GameObject button)
    {
        ...
        SceneRuntime.SceneLogic.ChangeRate(false);//true
    ...
    public void SetWeiXinImage(string _url)
    {
        if (null != m_FaceTexture)
            m_FaceTexture.gameObject.GetComponent<SpriteUrl>().SetAsyncImage(_url);
    }
