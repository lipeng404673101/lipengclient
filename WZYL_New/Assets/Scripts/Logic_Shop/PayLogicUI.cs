...
public class PayLogicUI : BaseWnd
{
    ...
    void DataWeiXin()
    {
        string tagUrl = "http://39.108.5.10:81/Apx/WeiXin.aspx";
        Debug.Log(tagUrl);
        CookieCollection cookies = new CookieCollection();//如何从response.Headers["Set-Cookie"];
        HttpWebResponse resp = HttpWebResponseUtility.CreateGetHttpResponse(tagUrl, null, null, cookies);
        Stream s = resp.GetResponseStream();
        StreamReader sr = new StreamReader(s);
        string text = sr.ReadToEnd();
        sr.Close();
        s.Close();
        string[] dataone = text.Split('|');
        Transform onn= m_BaseTrans.Find("Group").GetChild(0).GetChild(0);
        for (int i = 0; i < dataone.Length; i++)
        {
            
            string[] data = dataone[i].Split(':');
            GameObject obj = GameObject.Instantiate(Resources.Load("People") as GameObject);
            obj.transform.SetParent(onn);
            obj.transform.position = Vector3.zero;
            obj.transform.localScale = Vector3.one;
            obj.transform.GetChild(0).GetComponent<UILabel>().text = data[0];
            obj.transform.GetChild(1).GetComponent<UILabel>().text = data[1];
        }
    }
...
// 			if (PlayerRole.PanleChoose==2) {
// 				Debug.Log (PlayerRole.PanleChoose);
// 				foreach (Transform a1 in GameObject.Find ("SceneUIRoot").transform.Find ("ParentPanel").transform) {
// 					if (a1.name == "HallRoomWind(Clone)") {
...
// 			else
// 			{
// 				GameObject.Find ("SceneUIRoot").transform.Find ("ParentPanel").transform.Find ("HallRoomWind(Clone)").gameObject.SetActive (true);
            // 			}
            /***以下自己加的****/
            if (GameObject.Find("InformationWind(Clone)") || GameObject.Find("SceneBtnWind(Clone)") != null)
            {
