...
public class ShopLogicMgr : BaseWnd
{
    ...
    public void Init()
    {
        ...
        UIEventListener.Get(m_BaseTrans.GetChild(22).gameObject).onClick = OnClickbackbtn;
    ...
    void OnClickBackMsg(GameObject go)
    {
        int Panl = 0;
        GlobalAudioMgr.Instance.PlayOrdianryMusic(Audio.OrdianryMusic.m_BtnMusic);
        int jinbi = (int)(PlayerRole.Instance.RoleInfo.RoleMe.GetGlobel());
        string duibi = GameObject.Find("SceneUIRoot").transform.GetChild(1).transform.Find("ShopSysWndUI0(Clone)").transform.Find("Input_Zhifubao (2)").GetChild(0).GetComponent<UILabel>().text;
        string ChangeGlobel = GameObject.Find("SceneUIRoot").transform.GetChild(1).transform.Find("ShopSysWndUI0(Clone)").transform.Find("Input_Zhifubao (2)").GetChild(0).GetComponent<UILabel>().text;
        string UseName = GameObject.Find("SceneUIRoot").transform.GetChild(1).transform.Find("ShopSysWndUI0(Clone)").transform.Find("Input_Zhifubao (1)").GetChild(0).GetComponent<UILabel>().text;
        string Addres = GameObject.Find("SceneUIRoot").transform.GetChild(1).transform.Find("ShopSysWndUI0(Clone)").transform.Find("Input_Zhifubao (4)").GetChild(0).GetComponent<UILabel>().text;
        if (duibi != "" && UseName != "" && Addres != "")
        {
            Debug.Log(jinbi);
            int panle = int.Parse(ChangeGlobel) * 1000;
            if (100000 <= panle && panle <= jinbi)
            {
                if (UseName.Length <= 11)
                {
                    if (Addres.Length <= 129 && Addres.Length >= 11)
                    {
                        Cl_Cmd_ShopGoldItem ncb6 = new Cl_Cmd_ShopGoldItem();
                        ncb6.SetCmdType(NetCmdType.CMD_CL_ShopGoldItem);
                        ncb6.dwUserID = PlayerRole.Instance.RoleInfo.RoleMe.GetUserID();
                        ncb6.ChangeGlobel = (uint.Parse(ChangeGlobel) * 1000);
                        ncb6.UseName = UseName;
                        ncb6.Addres = Addres;
                        ncb6.IDEntity = "666666666666666666";
                        NetServices.Instance.Send<Cl_Cmd_ShopGoldItem>(ncb6);
                    }
                    else
                    {
                        GlobalHallUIMgr.Instance.ShowSystemTipsUI("支付宝账号长度不符", 2f, false);
                    }
                }
                else
                {
                    GlobalHallUIMgr.Instance.ShowSystemTipsUI("昵称过长", 2f, false);
                }
            }
            else
            {
                GlobalHallUIMgr.Instance.ShowSystemTipsUI("金额错误", 2f, false);
            }
        }
        else
        {
            GlobalHallUIMgr.Instance.ShowSystemTipsUI("信息不完整", 2f, false);
        }
        return;
    }
