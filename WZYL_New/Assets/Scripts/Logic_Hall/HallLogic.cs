...
public class HallLogic:ILogic, ICmdHandler
{
    ...
    public bool Handle(NetCmdPack pack)
    {
        ...
        else if (pack.cmd.GetCmdType() == NetCmdType.CMD_LC_JoinTable)
        {
            ...
            Debug.Log("房间ID" + ncb.bTableTypeID);
    ...
    public void JoinRoom(byte roomid)//进入房间
    {
        Debug.Log(roomid);
        Room = roomid;
        if (true)//判断本地中是否存有此键值
        {
            ...
            Debug.Log("类型是什么nenen" + SceneMain.IsUserOldServer);
            //发送进入房间的命令到服务端去
        ...
        else
        {
            ...
            {
                GlobalHallUIMgr.Instance.ShowSystemTipsUI("不能频繁点击", 2f, false);
            }
        }
    ...
    public void ROOM()
    {
        ...
        GlobalEffectMgr.Instance.ShowLoadingMessage();
        Debug.Log("类型是什么" + SceneMain.IsUserOldServer);
        //发送进入房间的命令到服务端去
        if (SceneMain.IsUserOldServer)
        {
            Debug.Log("SceneMain.IsUserOldServer");
