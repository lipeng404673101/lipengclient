...
{
    SignInChestsInfo[]  m_SignInChestsInfo = new SignInChestsInfo[5];//箱子
    ...
    GameObject[] m_AgainSignInBtnBg = new GameObject[2];//补签按纽
   ...
   // HallLoginUI_Activity m_Parent;
	public void Init(GameObject go)
    {
        ...
        for (byte i = 0; i < m_SignInBtn.Length; ++i)
        {
            ...
            if (i == 0)
            {
                m_AgainSignInBtnBg[0] = m_BaseTrans.GetChild(8).GetChild(0).gameObject;//补签？
                m_AgainSignInBtnBg[1] = m_BaseTrans.GetChild(8).GetChild(2).gameObject;//补签已用完？
    ...
    void OnClickRepeatSignIn(GameObject go)//重复点击签到按钮？
    {
        GlobalAudioMgr.Instance.PlayOrdianryMusic(Audio.OrdianryMusic.m_BtnMusic);//音效
