...
public class HallLogicUI_Main:HallLoginUI_BaseWind
{
    ...
    Benefit m_benefit = new Benefit();//救济金？
   ...
   // Object              m_effectReward = new Object();
	...
	//	m_CenterObj.SetActive (false);
        WindID.m_state = HallLogicUIStatue.Hall_State.Hall_Mian;
        WindID.m_SmailWind = 0;
        string[] EffectRName = { "Fish_MainUI_Lianji ", "Fish_MainUI_Jingsai", "Fish_MainUI_Chuangguan" };//右边的特效
        string[] EffectLName = { "FishCrownLight","FishYachiLight", "FishSmokeEffect"  };//左边的特效

        
        for (byte i = 0; i < 3; ++i)
        {
            m_EffectLiftBtn[i] = ResManager.Instance.LoadObject(EffectLName[i], "HallRes/Prefab/Effect/UIFishEffect/", ResType.HallRes);
            m_EffectRigthBtn[i] = ResManager.Instance.LoadObject(EffectRName[i], "HallRes/Prefab/Effect/UIModeEffect/", ResType.HallRes);//特效所在位置 
        }
    ...
    void InitCenterBtn()
    {
        ...
        for (byte i = 0; i < 3; ++i)
        {
            m_CenterLiftBtn.m_Effect[i] =  GameObject.Instantiate(m_EffectLiftBtn[i]) as GameObject;
    ...
    {
		//中心左按钮startBtn_yu
        string[] BgSpName = { "", "MainHal_Match1", "MainHal_Coustoms1" };//MainHal_ManyPeople1
