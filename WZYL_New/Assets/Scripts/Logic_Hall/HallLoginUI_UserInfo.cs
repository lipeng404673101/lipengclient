...
public class HallLoginUI_UserInfo : HallLoginUI_BaseWind
{
    ...
    public void UpdateUserInfo()
    {
        if (WindObj.activeSelf != true)
            return;
        m_UINickName.text = PlayerRole.Instance.RoleInfo.RoleMe.GetNickName();//用户昵称
        m_labelgameid.text = PlayerRole.Instance.RoleInfo.RoleMe.GetUserID().ToString();
        Debug.Log(m_labelgameid.text);
        ...
        m_HeadTex.GetComponent<SpriteUrl>().SetAsyncImage(PlayerRole.Instance.RoleInfo.RoleMe.GetImageUrl());
        //经验显示
