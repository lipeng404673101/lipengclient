...
public class HallLoginUI_Infomation : HallLoginUI_BaseWind {
            ...
            case 9:     //修改昵称
                {
                    ...
                    PlayerRole.Instance.RoleInfo.ChangeNickName(m_ChangeUserInFInPut.value);//m_ChangeUserInFInPut.value
                                                                                            //按钮禁用
    ...
    void ShowWind(byte Type)//点击头像显示个人信息
    {
        Debug.Log("ShowWind");

        // 		foreach (Transform a in GameObject.Find("SceneUIRoot").transform.Find("ParentPanel").transform) {
        // 			if (a.name == "HallRoomWind(Clone)") {
        // 				a.gameObject.SetActive (false);
        // 			}
        // 		}
        GameObject.Destroy(GameObject.Find("HallRoomWind(Clone)"));//自己加的
    ...
    void UpdateInformationUIDate(InfmationWind type) {
        Debug.Log("UpdateInformationUIDate");
        switch (type) {
