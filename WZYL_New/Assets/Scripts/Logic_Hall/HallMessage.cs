    ...
    public void ShowMessageBox(string strMsg, MssageType type, byte RoomID = 0, uint ExData = 0)
    {
        ...
        switch (type)
        {
            ...
            case MssageType.GameRoom_Msg_2:   
                {
                    Debug.Log("MssageType.GameRoom_Msg_2");
				int i = 0;
...
//        }
      //  else
            curRate = 0;
		//需解锁[ff0000]{0}[-]倍率，您当前已解锁[ff0000]{1}[-]倍，是否消耗[ff0000]{2}[-]钻石解锁至[ff0000]{3}[-]倍？
 //       string strDec = string.Format(StringTable.GetString("join_room_failed3"), desRate, curRate, m_ConsumeSum, desRate);
		string strDec = string.Format(StringTable.GetString("join_room_failed1"), desRate);
        m_MsgBoxLabel[0].text = "[242858FF]" + strDec + "[-]";
		GameObject.Find ("SceneUIRoot").transform.Find ("ParentPanel").transform.Find("HallRoomWind(Clone)").gameObject.SetActive(true);
...
//					}
    ...
    {
		PlayerRole.PanleChoose = 1;
        GlobalAudioMgr.Instance.PlayOrdianryMusic(Audio.OrdianryMusic.m_CloseUI);
        Debug.Log("this is PayButton??????");
        GlobalHallUIMgr.Instance.ShowPayWnd(PayType.Gold);
        ShutDown();
    }
   public  void OnClickRecommendRoom(GameObject go)//加了个public
    {
        Debug.Log("OnClickRecommendRoom");
        GlobalAudioMgr.Instance.PlayOrdianryMusic(Audio.OrdianryMusic.m_CloseUI);
