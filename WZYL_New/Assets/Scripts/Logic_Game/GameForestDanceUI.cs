...
public class GameForestDanceUI : BaseWnd
{
    ...
    void Init()
    {
        ...
        //SceneObjMgr.Instance.UIPanelTransform.GetChild(8).gameObject.SetActive(false);

        UIEventListener.Get(m_BaseTrans.GetChild(8).gameObject).onClick = EventBack;//森林退出按钮
    ...
    void InitTitle()
    {
        ...
        UIEventListener.Get(hostBtnObj[0]).onClick = EventUpHost;   //senlin 上庄
        UIEventListener.Get(hostBtnObj[1]).onClick = EventDownHost;  //senlin 下庄
        ...
        hostBtnObj[0].SetActive(false);//true   显示上庄图标
    ...
    void InitWeagerWnd()
    {
        ...
        weagerArr = new uint[] { 1000, 5000, 10000, 50000, 100000 };
        for (byte i = 0; i < 12; i++)
        {
    ...
    {      
        ...
        //else
        //{
        m_GoldLabel.GetComponent<UILabel>().text = string.Format("{0:N3}", (goldNum / (double)1000f)).ToString().Replace(",", "");
    ...
    void EventContinue(GameObject go)
    {
        ...
        if(PlayerRole.Instance.RoleMiNiGame.m_DialGame.OnAddGlobelByLog())
        {
            for (int i = 0; i < 12; i++)
            {
                weagerWnd[i].myWeagerNum = PlayerRole.Instance.RoleMiNiGame.m_DialGame.TabalGlobelArarNew[i];
                Debug.Log(" relo.TabalGlobelArar[i]" + PlayerRole.Instance.RoleMiNiGame.m_DialGame.TabalGlobelArarNew[i]);
                PlayerRole.Instance.RoleMiNiGame.m_DialGame.TabalGlobelArarNew[i] = 0;
            }
        }
    ...
    {                    
        ...
        else if (TableStates == DialTableStates.DTS_End)
        {
            ...
            Debug.Log("求值：" + tableResult + AreaData + TableGlobelInfo);
    ...
    public void UpdateTotalWeager(DialTableUpdateInfo TableGlobelInfo, DialTableStates TableStates)
    {
        for (byte i = 0; i < 12; i++)
        {
            totalWeagerLabel[i].text = (TableGlobelInfo.GlobelNum[i]/1000f).ToString();
        }
    }
    ...
    public void UpdateMyWeager(uint[] TableGlobelInfo)
    {
        for (byte i = 0; i < 12; i++)
        {
            weagerWnd[i].myWeagerLabel.text = (TableGlobelInfo[i]/1000f).ToString();
        }
        SetPlayerGold();
    ...
    void SetResultValue(uint tableResult, DialTableAreaData AreaData, DialTableUpdateInfo TableGlobelInfo)
    {
        ...
        for (byte i = 0; i < 12; i++)
        {
            ...
            Debug.Log("myWeagerNum" + (int)weagerWnd[i].myWeagerNum);
            if (i == rateIndex)
            {
        ...
        if (!isHost)
        {
            ...
            Debug.Log("三生三世十里桃花");
            myResultLabel.text = (myResult/1000f).ToString();
            Debug.Log("jeiguo myResultLabel.text +++" + myResultLabel.text);
            if (myResult >= 0)
                myResultLabel.text = "+" + (myResult/1000f);
           
            hostResultLabel.text = hostResult.ToString();
            if (hostResult >= 0)
                hostResultLabel.text = "+" + (hostResult/1000f);
        }
        ...
        else
        {
            ...
            hostResultLabel.text = (hostResult/1000f).ToString();
            myResultLabel.text = (hostResult/1000f).ToString();
            if (hostResult >= 0)
            {
                hostResultLabel.text = "+" + (hostResult/1000f);
                myResultLabel.text = "+" + (hostResult/1000f);
            }
        }
        GlobalHallUIMgr.Instance.GameShare.AddGlod(myResult);
        Debug.Log("jeiguo +++" + myResult);
    }
    ...
    public void UpdateHost(DialBankerUserInfo info)
    {
        ...
        else
        {
            ...
            if (hostName.Equals(PlayerRole.Instance.RoleInfo.RoleMe.GetNickName()))
            {
                ...
                hostBtnObj[1].gameObject.SetActive(true);//true
            ...
            else
            {
                ...
                {
                    hostBtnObj[0].SetActive(true);//true
                    hostBtnObj[1].SetActive(false);
                    hostBtn[0].SetActive(false);
                    hostBtn[1].SetActive(false);
                    hostBtn[2].SetActive(false);
    ...
    public void BackToMain()
    {
        ...
        //SceneObjMgr.Instance.UIPanelTransform.GetChild(8).gameObject.SetActive(true);
    }
