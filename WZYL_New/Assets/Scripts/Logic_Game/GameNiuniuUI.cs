...
public class GameNiuniuUI : BaseWnd
{
    ...
    void InitHostLine()
    {
        ...
        hostBtn[3].SetActive(false);//true
        isHost = false;
        isInLine = false;
        hostInfo = null;
        isMasterHost = true;
    ...
    void InitLevelBtn()
    {
        ...
        levelArr = new int[] { 1000, 10000, 50000, 500000, 1000000 };
    ...
    public void SetMyGoldNum()
    {
        if (!isAnim)
        {
            //double goldNum = PlayerRole.Instance.RoleInfo.RoleMe.GetGlobel();
            //if (goldNum >= 10000)
            //{
            // m_HeadTrans.GetChild(8).GetComponent<UILabel>().text = (goldNum / 10000) + "." + (goldNum / 1000 % 10) + " 万";
            //  m_HeadTrans.GetChild(8).GetComponent<UILabel>().text = goldNum;
            //}
            //else
            //{
            //m_HeadTrans.GetChild(8).GetComponent<UILabel>().text =string .Format ("{0:N3}", (goldNum /1000f).ToString().Replace(",",""));
            m_HeadTrans.GetChild(8).GetComponent<UILabel>().text = string.Format("{0:N3}", (PlayerRole.Instance.RoleInfo.RoleMe.GetGlobel() / (double)1000f)).ToString().Replace(",", "");
            Debug.Log("Set My Gold Num is " + m_HeadTrans.GetChild(8).GetComponent<UILabel>().text);
            //}
        }
    }
    void InitPlayerInfo()
    ...
    void EventChangeLevel(GameObject go)//下注的按钮
    {
    ...
    void EventWagerChoose(GameObject go)
    {
        if (isInWeager)
        {
            for (byte i = 0; i < 4; i++)
            {
                if (go == m_PoolTrans.GetChild(i).gameObject)//下注的区域
                {
                    poorIndex = i;
    ...
    void ShowResult()                   //显示结算ui
    {
        ...
        if (myWeager[0] + myWeager[1] + myWeager[2] + myWeager[3] != 0)
        {
            ...
            if (total >= 0)
            {
                ...
                m_CalculateTrans.GetChild(1).GetComponent<UILabel>().text = "+" + (total / 1000f);
            }
            else if (total < 0)
            {
                ...
                //else
                //    m_CalculateTrans.GetChild(1).GetChild(0).gameObject.SetActive(false);
                m_CalculateTrans.GetChild(1).GetComponent<UILabel>().text = (total / 1000f).ToString();
            }
    ...
    void EventCloseResult(GameObject go)        //关闭结算UI
    {
        ...
        if (!isHost)
        {
            ...
            //m_PoolTrans.GetChild(0).GetChild(3).GetChild(0).GetComponent<UILabel>().text = "0";
            //m_PoolTrans.GetChild(1).GetChild(3).GetChild(0).GetComponent<UILabel>().text = "0";
            //m_PoolTrans.GetChild(2).GetChild(3).GetChild(0).GetComponent<UILabel>().text = "0";
            //m_PoolTrans.GetChild(3).GetChild(3).GetChild(0).GetComponent<UILabel>().text = "0";
        }
    ...
    void EventDownHost(GameObject go)
    {
        if (PlayerRole.Instance.RoleMiNiGame.m_NiuNiuGame.OnRoleCanelBanker())
        {
            ...
            hostBtnObj[0].SetActive(false);//true
            hostBtnObj[1].SetActive(false);
        }
    }
    ...
    public void UpdateHost(BankerUserInfo info)
    {
        ...
        else
        {
            ...
            hostGoldNumLabel.text = (goldNum / (double)1000f).ToString();
            //if (goldNum > 10000)
            //{
            ...
            if (hostName.Equals(PlayerRole.Instance.RoleInfo.RoleMe.GetNickName()))
            {
                ...
                hostBtn[0].SetActive(false);
                hostBtn[1].SetActive(false);
                hostBtn[2].SetActive(false);//true
                hostBtn[3].SetActive(false);
            }
            else
            {
    ...
    void UpdateVipInfo(VipSeatList list)
    {
        for (byte i = 0; i < list.List.Length; i++)
        {
            if (list.List[i].dwUserID != 0)
            {
                ...
                vipSeat[i].moneyLabel.text = (moneyNum / (double)1000f).ToString();
                //if (moneyNum > 10000)
    ...
    public void VipWeager(byte seat, uint addGlobal, byte index, uint allGlobal)
    {
        ...
        vipSeat[seat].moneyLabel.text = (allGlobal / 1000f).ToString();
        //if (allGlobal > 10000)
    ...
    public void VipGlobalChange(uint id, byte seat, uint globalSum)
    {
        if (id != 0)
        {
            vipSeat[seat].moneyLabel.text = (globalSum / 1000f).ToString();
            //if (globalSum > 10000)
                            ...
                            if (vipInfo[j].userId != 0 && vipInfo[j].userId != PlayerRole.Instance.RoleInfo.RoleMe.GetUserID())
                            {
                                if (vipInfo[j].weagerIndex[i - 1])
                                {
                                    if (goldNum < 5)
                                    {
                                        ...
                                        {
                                            case 0:
                                                vector = new Vector3(-97, 465, 0);
                                                break;
                                            case 1:
                                                vector = new Vector3(-176, 368, 0);
                                                break;
                                            case 2:
                                                vector = new Vector3(-168, 29, 0);
                                                break;
                                            case 3:
                                                vector = new Vector3(-89, 163, 0);
                                                break;
                                            case 4:
                                                vector = new Vector3(698, 471, 0);
                                                break;
                                            case 5:
                                                vector = new Vector3(772, 368, 0);
                                                break;
                                            case 6:
                                                vector = new Vector3(772, 262, 0);
                                                break;
                                            case 7:
                                                vector = new Vector3(693, 165, 0);
                                                break;
                                            default:
                                                vector = Vector3.zero;
                                                break;
        ...
        if (state == 2)
        {
            ...
            for (byte i = 0; i < oldVipInfo.List.Length; i++)
            {
                ...
                else
                {
                    ...
                    vipSeat[i].moneyLabel.text = (global / 1000f).ToString();
                    //if (global > 10000)
    ...
    public void UpdateTotalMoney(ulong[] date, int state = 1)          //更新总下注
    {
        for (byte i = 0; i < totalNum.Length; i++)
        {
            ...
            //else
            //{
            m_PoolBtn[i].transform.GetChild(2).GetComponent<UILabel>().text = (totalNum[i] / 1000f).ToString();//每个池子的总数量
    ...
    public void UpdateMyWeagerNum()                     //更新我的下注
    {
        ...
        //else
        m_PoolTrans.GetChild(poorIndex).GetChild(3).GetChild(0).GetComponent<UILabel>().text = (myWeager[poorIndex] / 1000f).ToString();//显示区域自身下注金币
        ...
        //else
        //{
        m_PoolBtn[poorIndex].transform.GetChild(2).GetComponent<UILabel>().text = (totalNum[poorIndex] / 1000f).ToString();
        ...
        if (index != 0)
        {
            ...
            else
            {
                if (result[index] > result[0])
                {
                    ...
                    trans.GetChild(1).GetChild(4).GetComponent<UILabel>().text = "+" + (resultNum[index - 1] / 1000f);
                    trans.GetChild(1).GetChild(3).GetComponent<UILabel>().text = "*" + resultRate[index];
                }
                else
                {
                    resultNum[index - 1] = -resultRate[0] * myWeager[index - 1];
                    trans.GetChild(1).GetChild(4).GetComponent<UILabel>().text = (resultNum[index - 1] / 1000f).ToString();
...
class HostItem : BaseWnd
{
    ...
    public Transform InitItem(Transform trans)
    {
        ...
        nameLabel.text = name;
        goldNumLabel.text = (goldNum / 1000f).ToString();
        return m_BaseTrans;
    }
}
...
class NormalItem : BaseWnd
{
    ...
    public Transform Init(Transform trans)
    {
        ...
        nameLabel.text = name;
        goldLabel.text = (gold / 1000f).ToString();
        //if (gold > 10000)
