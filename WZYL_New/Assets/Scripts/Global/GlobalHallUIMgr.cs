...
public class HeadWind : GlobalBaseWind
{
    ...
    public void SetWeixinHeader(string _url)
    {
        if(null!=  GameObject.Find("HallWindBtns(Clone)"))
        {
            m_ImageUrl = PlayerRole.Instance.RoleInfo.RoleMe.GetImageUrl();
        }else
        {
            m_ImageUrl = _url;
        }
        Debug.Log("设置捕鱼头像：" + m_ImageUrl);
        
        if (null != m_HeadTexture)
        {
            //开始获取头像
            Debug.Log("初始化");
            m_HeadTexture.gameObject.GetComponent<SpriteUrl>().SetAsyncImage(m_ImageUrl);
        }
        else
        {
            Debug.Log("还没有初始化");
    ...
    public void SetHeadInf()
    {
        ...
        //设置头像
        //Debug.Log("设置头像:" + m_HeadTexture.gameObject.activeSelf);
        //m_HeadTexture.transform.GetComponent<SpriteUrl>().SetAsyncImage(m_ImageUrl);
        //m_HeadTexture.mainTexture = GlobalHallUIMgr.Instance.m_HeadTextureUI[ficeID];
    ...
    float GetCurExperience()
    {
        ...
        if (PlayerRole.Instance.RoleInfo.RoleMe.GetLevel() < Experience.Exp.Length)
        {
            ...
            //Debug.Log("当前的经验：" + PlayerRole.Instance.RoleInfo.RoleMe.GetExp());
...
public class GlobalHallUIMgr : Singleton<GlobalHallUIMgr>
{
    ...
    public void ShowPayWnd(PayType type)
    {
        ...
        daili.UserID = PlayerRole.Instance.RoleInfo.RoleMe.GetUserID();
    ...
    public void SetMyHeader(string _url)
    {
        m_HeadWind.SetWeixinHeader(_url);
    }
