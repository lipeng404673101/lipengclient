...
public class Audio
{
    ...
    public enum EffectGetType
    {
        EffectGetGold = 0,//爆炸金币音效
    }
}
class GlobalAudioMgr : Singleton<GlobalAudioMgr>
{
    ...
    List<Audio> m_listGetAudio = new List<Audio>();
    ...
    System.Collections.IEnumerator Initaudio(object dd)
    {
        ...
        {
            Audio effectAudio = new Audio();
            effectAudio.AudioName = string.Format("w{0}", w);
            effectAudio.CreateAudioClip();
            if (effectAudio.MyAudioClip != null)
                m_listGetAudio.Add(effectAudio);
            else
                break;
        }
        InitLogic.EndInit();
    ...
    public void PlayGetMusic(Audio.EffectGetType GetMusicIndex/*音效的类型*/, bool bLoop = false/*是否循环*/, bool bStopLoop = false/*是否停止循环主要是用来听上个音效*/, float volume = 1)
    {
        if (bStopLoop)
        {
            if (LoopAudio == null)
        ...
        if (bLoop)
        {
            ...
            LoopAudio.Stop();
            LoopAudio.loop = true;
            LoopAudio.clip = m_listGetAudio[(int)GetMusicIndex].MyAudioClip;
            LoopAudio.Play();
            LoopAudio.volume = m_BgValue * volume;
        }
        else
        {
            NGUITools.PlaySound(m_listGetAudio[(int)GetMusicIndex].MyAudioClip, volume);
        }
    }
