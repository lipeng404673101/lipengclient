...
public class LogonLogic : ILogic, ICmdHandler
{
    ...
    {
        if (GameObject.Find("SceneUIRoot").transform.Find("ParentPanel").Find("LogonWidget1(Clone)").GetChild(1).GetChild(2).gameObject.active)
        {
            string name = GameObject.Find("SceneUIRoot").transform.Find("ParentPanel").Find("LogonWidget1(Clone)").GetChild(5).GetChild(0).GetComponent<UIInput>().value;
            string value = GameObject.Find("SceneUIRoot").transform.Find("ParentPanel").Find("LogonWidget1(Clone)").GetChild(5).GetChild(1).GetComponent<UIInput>().value;
            PlayerPrefs.SetString("Type", "sure");
            PlayerPrefs.SetString("Name", name);
            PlayerPrefs.SetString("Password", value);
        }
        else
        {
            PlayerPrefs.SetString("Type", "sorry");
        }
    ...
    void _WeiXinLogon(string Str, string Token, string imageurl, string nickName)
    {
        m_OpenID = Str;
        m_Tonken = Token;
        m_ImageUrl = imageurl;
   ...
   public void _PhoneLogon(UInt64 PhoneNumber,UInt32 Password,string NewPassword,bool IsFirOrSec)
    ...
    void SendLogonData()
    {
        switch (m_State)
        {
            ...
            case LogonState.LOGON_REGISTER_LOGINNING:
                ...
                else
                {
                    ...
                    if (!FishConfig.Instance.m_ErrorString.CheckStringIsError(FishDataInfo.AccountNameMinLength, FishDataInfo.AccountNameLength, ncb.AccountName, StringCheckType.SCT_AccountName))
                    {
                        ...
                        ncb.dwUserID = uint.Parse(GlobalLogon.Instance.AccountData.TempAccountInfo.INVATE);
                        //发送命令
            ...
            case LogonState.LOGON_WEIXIN_LOGINNING:
                { 
                    SendWeiXinLogon(m_OpenID, m_ImageUrl, m_nickName);
                }
                break;
    ...
    System.Collections.IEnumerator WaitConnect(object obj)
    {
        ...
        if (m_State == LogonState.LOGON_SDK_LOGINNING)
        {
            ...
            //LogonState.LOGON_WEIXIN_LOGINNING
            SetState(LogonState.LOGON_WEIXIN_LOGINNING);
            SendLogonData();
#endif
        }
    ...
    void SendSDKLogonData()
    {
        ...
        Debug.Log("cmdLogon.channelInfo.uid" + cmdLogon.channelInfo.uid);
    ...
    public bool Handle(NetCmdPack pack)
    {
        if (pack.cmd.GetCmdType() == NetCmdType.CMD_LC_AccountOnlyID)
        {
            ...
            Debug.Log(ncb.LogonTypeID+"这个是返回的登录消息");
            switch(ncb.LogonTypeID)
            {
                case 0:
                    {
                        GlobalHallUIMgr.Instance.ShowSystemTipsUI("邀请码绑定失败", 2f, false);
        ...
        {
            LC_Cmd_CheckVersionError ncb = (LC_Cmd_CheckVersionError)pack.cmd;//ｐａｃｋ。ｃｍｄ
   ...
   public void OnBeginWeiXinLogon()
    ...
    public void OnHandleQQOrWeiXinLogonResult(string Str, string Token, string image_url, string nick_name)
    {
        ...
        {
            Debug.Log("走这里");
            _WeiXinLogon(Str, Token, image_url, nick_name);
            //SendWeiXinLogon(Str);
        }
    }
    ...
    //QQ 和微信登陆的回调函数
    void SendWeiXinLogon(string Code, string image_url, string nick_name)
    {
        Debug.Log("发送微信进入大厅的消息");
