...
public class SceneGoldEffect
{
    ...
    private Dictionary<int, int> m_Up = new Dictionary<int, int>()
    {
        { 9,40},
        { 18,50},
        {20,60 },
    ...
    public void ShowGoldEffect(CatchedData cd, Fish fish)
    {
        ...
        //如果是BOSS播放特效震屏
        //SceneRuntime.SceneLogic.UseSkill(SkillType.SKILL_DISASTER);
        //20 1 3 19 
        if (!MatchJudge.IsMatch() && cd.ClientSeat == SceneRuntime.MyClientSeat)
        {
            int curRand = Random.Range(0, 25);
            if (curRand == fish.FishType && m_YunshiTime > 300.0f)
            {
                //触发陨石效果
                m_YunshiTime = 0.0f;
                SceneRuntime.SceneLogic.UseSkill(SkillType.SKILL_DISASTER);
            }
            else if (m_YunshiTime > 3.0f)
            {
                if (m_Up.ContainsKey(fish.FishType))
                {
                    Debug.Log("打下类型的鱼+" + fish.FishID);
                    SceneRuntime.SceneLogic.ShowBoomGold((int)fishGold, fish);
                    ScreenShake.isshakeCamera = true;
                    GlobalAudioMgr.Instance.PlayGetMusic(Audio.EffectGetType.EffectGetGold);
                }
            }
        }

        //Debug.Log("打下类型的鱼+" + fish.FishID);
        //SceneRuntime.SceneLogic.ShowBoomGold(20, fish);
        //ScreenShake.isshakeCamera = true;
        //GlobalAudioMgr.Instance.PlayGetMusic(Audio.EffectGetType.EffectGetGold);

        //Debug.Log("打下类型的鱼+" + fish.FishID);
        //SceneRuntime.SceneLogic.ShowBoomGold(fish.FishID, fish);
        //ScreenShake.isshakeCamera = true;
        //根据不同类型的鱼播放不同的音效
    ...
    void ShowGoldNumLabel(uint _Score, Vector3 Pos, Fish fish, bool bScale = true)
    {
        ...
        label.SetPos(Pos);
        double _Scores = (double)(_Score / 1000f);
        label.SetText(string.Format("{0:N3}", _Scores).ToString().Replace(",", ""));//鱼死亡后掉落的金币
        if (fish.IsBossFish())
        {
            label.LifeTime = 3.0f;
   ...
   void ShowGetGold(int gold, Vector3 pos)
    {
      ...
      //  getGold.ResetScale();
        ...
        getGold.SetPos(pos);
        getGold.SetTweenPos();
        double golds = (double)(gold / 1000f);
        getGold.SetText(string.Format("{0:N3}", golds).ToString().Replace(",", ""));//黑框的金币
    ...
    public void Update(float delta)
    {
        ...
        m_YunshiTime += Time.deltaTime;
        if(m_YunshiTime > 10000.0f)
        {
            m_YunshiTime = 0.0f;
        }
