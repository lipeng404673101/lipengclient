...
public class SceneWonderful : BaseWnd
{
    ...
    public void ShowBoomGoldUI(int gold ,Fish fish)
    {
        if (m_BaseWndObject.activeSelf == true)
        {
            return;
        ...
        m_Scroe = (double)gold;
        m_BaseWndObject.SetActive(true);
        m_SocreLabel.text = string.Format("{0:N3}",(m_Scroe / (double)1000)).ToString().Replace(",", "");
    ...
    public bool UpDataUIScroe(float deltaTime)
    {
        ...
        if (m_bNumAinm)
        {
            ...
            if (m_SocreLabel != null && ++m_Index > rate)
            {
                ...
                double.TryParse(str, out numScroe);
                numScroe = numScroe * 1000;
                numScroe += AddNum;
                if (numScroe >= m_Scroe)
                {
                    m_bNumAinm = false;
                ...
                m_SocreLabel.text = string.Format("{0:N3}",( numScroe / (double)1000)).ToString().Replace(",", "");
