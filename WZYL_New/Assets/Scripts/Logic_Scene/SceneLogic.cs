...
public class SceneLogic:ILogic, ICmdHandler
{
    ...
    //播放爆炸金币的特效
    public void ShowBoomGold(int gold , Fish fish)
    {
        m_LogicUI.showBoomGoldUI(gold, fish);
    }
    ...
    public void PlayerJoin(NetCmdPack pack)
    {
        ...
        pd.playerData.ImageUrl = ncp.PlayerInfo.Image_Url;
    ...
    public void ResetPlayerData(JoinRoomData jrd, bool bFirst)
    {
        ...
        pPlayer.playerData.ImageUrl = pMe.GetImageUrl();
    ...
    public void ChangeDestLauncher(byte launcherType)
    {
        ...
        ncb.Seat = SceneRuntime.ClientToServerSeat(m_PlayerMgr.MyClientSeat);
        ncb.LauncherType = launcherType;
        Send<NetCmdChangeLauncherType>(ncb);//这里是上传炮台现在类型的信息
		GameObject.Find ("SceneUIRoot").transform.Find ("ParentPanel").transform.Find ("Skill_Bar(Clone)").transform.Find("LockedBtn").transform.localPosition = new Vector3(-417.4f,-270f,0f);
    }
