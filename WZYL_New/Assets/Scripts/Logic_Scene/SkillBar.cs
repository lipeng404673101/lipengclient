...
public class SkillBar : BaseWnd
{
    ...
    public void UpdateSkillState()
    {
                ...
                if (Idx != 255)
                {
                   ...
                   // m_UIButton[0].isEnabled = true;
                    ...
                    if (m_UIButton[0].gameObject.activeSelf == false)
                    {
						...
						//Debug.Log ("坐标1"+ m_RightBtnTrans [0].localPosition);
						//Debug.Log ("坐标2"+ m_RightBtnTrans [1].localPosition);
                    }
		...
		GameObject.Find ("SceneUIRoot").transform.Find ("ParentPanel").transform.Find ("Skill_Bar(Clone)").transform.Find("LockedBtn").transform.localPosition = new Vector3(-417.4f,-270f,0f);//这里是锁定按钮的位子
    }
...
//			
                ...
                if (pShop != null)
                {
                    if (pShop.ShopItemMap.ContainsKey(OnlyID))
					{
                       // GlobalHallUIMgr.Instance.ShowConfirmBuyWnd(OnlyID, shopID, itemSum);
						PlayerRole.Instance.ShopManager.ShopItem (shopID, OnlyID, itemSum);
    ...
    void OnClickLocked(GameObject go)//点击锁定按钮
    {
    ...
    {
		Debug.Log ("OnClickUnlockedUnfoldUI");
        GlobalAudioMgr.Instance.PlayOrdianryMusic(Audio.OrdianryMusic.m_BtnMusic);
