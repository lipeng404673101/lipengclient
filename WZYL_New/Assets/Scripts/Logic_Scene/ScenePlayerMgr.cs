...
public class ScenePlayerMgr
{
    ...
    public bool PlayerJoin(PlayerExtraData player, byte clientSeat, byte rateIdx, byte launcherType, bool valid)
    {
        ...
        if (clientSeat != MyClientSeat)
        {
            ...
            sp.Launcher.SetWeiXinImage(player.playerData.ImageUrl);
        }
        else
        {
            ...
            //获取头像
            Debug.Log("开始设置头像:" + player.playerData.ImageUrl);
            GlobalHallUIMgr.Instance.SetMyHeader(player.playerData.ImageUrl);
        }
        //获取金币
    ...
    public void Update(float delta)//锁定
    {
       // -------------------------------------
    ...
    public void ChangeLauncher(NetCmdPack pack)
    {
        ...
        {
            Debug.Log("zheli");
            SceneRuntime.CheckLauncherValid(130, out clientLauncherType, out launcherValid);
