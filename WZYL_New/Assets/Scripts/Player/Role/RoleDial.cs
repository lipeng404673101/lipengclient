...
class RoleDial
{
    ...
    //更新 修改区域金币
    private UInt32[] TabalGlobelArar = new UInt32[FishDataInfo.MAX_Dial_ClientSum];
    public   UInt32[] TabalGlobelArarNew = new UInt32[FishDataInfo.MAX_Dial_ClientSum];//添加参数 记录下注
    ...
    public UInt32 GetRoleAddGlobelSum()
    {
        UInt32 AddGlobel = 0;
        for (int i = 0; i < FishDataInfo.MAX_Dial_ClientSum; ++i)
    ...
    public UInt32 GetRoleAddPoolSum(Byte num)
    {
        TabalGlobelArar[num] = TableGlobelSum[num];
        return TabalGlobelArar[num];
        //UInt32 poolNum = 3000000;
        //for (Byte i = 0; i < FishDataInfo.MAX_Dial_ClientSum; ++i)
        //{
        //    poolNum += TableGlobelSum[i];
        //}
        //return poolNum;
    ...
    public bool OnAddGlobel(UInt32 AddGlobel,Byte Index)
    {
        ...
        //区域限制金币
        UInt32[] poolGoldSum = new UInt32[12];
        poolGoldSum[Index] = GetRoleAddPoolSum(Index);
        UInt32 poolMaxSum = 3000000;

        if( poolGoldSum[Index]+AddGlobel  > poolMaxSum)
        {
            tagUserOperationEvent pUOM = new tagUserOperationEvent(UserOperateMessage.UOM_Dial_AddGlobel_Failed_3, (poolMaxSum / 1000).ToString());
            MsgEventHandle.HandleMsg(pUOM);
            return false;
        }
    ...
    public bool OnAddGlobelByLog()
    {
        ...
        {
            AddGlobel += LogTableGlobelSum[i];
            TabalGlobelArarNew[i] = LogTableGlobelSum[i];//记录下注
        }
    ...
    public bool OnHandleTableJoinBegin(NetCmdBase obj)
    {
        ...
        //NGUIDebug.Log(TableGlobelSum[i] + "下注信息2222222");
