...
class RoleNiuNiu
{
    ...
    //添加参数  记录区域分数
    private UInt32[] TableGlobelArar = new UInt32[FishDataInfo.MAX_NIUNIU_ClientSum];
    ...
    public UInt32 GetRoleAddPoolSum(byte Num)
    {
        // UInt32 gold = 0;
        //  for (Byte i = 0; i < FishDataInfo.MAX_NIUNIU_ClientSum; ++i)
        TableGlobelArar[Num] = TableGlobelSum[Num];
        return TableGlobelArar[Num];
    }
    //牛牛 管理类
    //进入牛牛房间
    ...
    //public bool IsLimit(UInt32 AddGlobel, Byte Index)
    //{
    //    UInt32 AllGlobelSum = GetRoleAddAllGlobelSum();
    //    if (Index<= FishDataInfo.MAX_NIUNIU_ClientSum && AddGlobel <= 3000000)
    //    {
    //        AllGlobelSum += AddGlobel;
    //        return true;
    //    }else
    //    {
    //        AllGlobelSum = 3000000;
    //        return false;
    //    }
    //}
    ...
    public bool OnAddGlobel(UInt32 AddGlobel, Byte Index)
    {
        ...
        //区域限制金币
        UInt32[] poolGoldSum = new UInt32[4];
        ...
        {
            tagUserOperationEvent pUOM = new tagUserOperationEvent(UserOperateMessage.UOM_NiuNiu_AddGlobel_Failed_3, (poolMaxSum/1000).ToString());
            MsgEventHandle.HandleMsg(pUOM);
            return false;
        }
        // }
