...
//        }
    ...
    public bool HandleCmd(NetCmdBase obj)
    {
        ...
        switch (obj.GetCmdType())
        {
            ...
            case NetCmdType.CMD_LC_TableChangeRoleGlobeDown:
                return HandleOtherRoleGobleDownNumChange(obj);
    ...
    bool HandleOtherRoleGobleDownNumChange(NetCmdBase obj)
    {
        Debug.Log("shuru");
        LC_Cmd_TableChangeRoleGlobeDown ncb = (LC_Cmd_TableChangeRoleGlobeDown)obj;
        //修改远程玩家的等级
        if (m_TableRole.ContainsKey(ncb.dwDestUserID))
        {
            m_TableRole[ncb.dwDestUserID].SetGlobel(Convert.ToUInt32(m_TableRole[ncb.dwDestUserID].GetGlobel() - ncb.dwGlobelSum));
