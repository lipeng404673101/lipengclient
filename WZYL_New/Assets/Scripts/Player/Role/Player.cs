...
public class RoleBase //玩家的基类
{
    ...
    public virtual string GetImageUrl() {
        return "http://wx.qlogo.cn/mmopen/EVxJeWK7jLpxBAzboIO94qNehTn1TBhC4KjMFhBj7sQfNfJrpPTue6Ph869ccPU4cVUu6KvT17LRTR5OtRfDjicyqCGib7ricH1/0";
    }
...
class RoleMe : RoleBase //本人 自己的数据类
{
    ...
    public virtual void SetRoleInfo(tagRoleInfo pRoleInfo) {
        Debug.Log("拉取设置用户信息");
        if (null == pRoleInfo) {
            Debug.Log("设置用户信息为空：null");
        } else {
            Debug.Log("头像：" + pRoleInfo.ImageUrl);
        }
        RoleInfo = pRoleInfo;
    }
    ...
    public override void SetLevel(UInt16 wLevel)//设置等级
    {
        //if (RoleInfo == null) //更新
...
class TableRole : RoleBase //桌子上的玩家数据类
{
    ...
    public override string GetImageUrl() {
        if (RoleInfo == null)
            return base.GetImageUrl();
        return RoleInfo.Image_Url;
    }
    public virtual void SetImageUrl(string url) {
        RoleInfo.Image_Url = url;
    }
    public override UInt32 GetUserID() {
        if (RoleInfo == null)
            return base.GetUserID();
